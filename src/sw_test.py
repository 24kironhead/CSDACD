...
from utils.metrics import Precision, Recall, F1Score, Accuracy, IoU
...
def main():
    ...
    prec, rec, f1, acc, iou = Precision(mode='accum'), Recall(mode='accum'), F1Score(mode='accum'), Accuracy(mode='accum'), IoU(mode='accum')
    ...
    try:
        for i, path in enumerate(iglob(osp.join(args['gt_dir'], args['glob']))):
            ...
            acc.update(cm, gt)
            iou.update(cm, gt)
            logger.show("{:>4} Precision: {:.4f} Recall: {:.4f} F1: {:.4f} IoU: {:.4f} OA: {:.4f} \n".format(
                i, prec.val, rec.val, f1.val, iou.val, acc.val
            ))
